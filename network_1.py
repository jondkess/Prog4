...
class Host:
    ...
    def udt_receive(self):
        ...
        if pkt_S is not None:
            if NetworkPacket.from_byte_S(pkt_S).prot_S != 'control':
...
class Router:
    ...
    def send_routes(self, i):
        ...
        p = NetworkPacket(0, 'control', Message.to_byte(self))
        try:
    ...
    def print_routes(self):
        ...
        print("Table {} Cost to".format(self.name))
       ## print("  "+" ".join(str(i) for i in range(len(self.intf_L))))
        print("  "+" ".join(str(i) for i in self.rt_tbl_D.keys()))
...
class Message:
    def __init__(self, update_table, origin):
        self.update_table = update_table
        self.origin = origin

    def to_byte(curRouter):
        msg = ""
        for host in curRouter.rt_tbl_D.keys():
                for interface, cost in curRouter.rt_tbl_D.get(host).items():
                    msg+="{}-{}:{},".format(host, curRouter.name, cost)
        return msg
    ...
    def from_byte(self, msg):
        ...
        while(len(msg) > 0):
            ...
            if matchObj:
                ...
                msg = msg[1:]
        return self(route_table, origin)