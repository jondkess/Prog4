...
if __name__ == '__main__':
    ...
    router_a_rt_tbl_D = {'1': {1: 1}, '2': {0: 9}} # packet to host 1 through interface 0 for cost 1
    ...
    router_c_rt_tbl_D = {} # packet to host 1 through interface 0 for cost 1
    router_c = network.Router(name='C', 
                              intf_cost_L=[1,2], 
                              rt_tbl_D = router_c_rt_tbl_D, 
                              max_queue_size=router_queue_size)
    object_L.append(router_c)
    router_d_rt_tbl_D = {'3': {2: 1}} # packet to host 1 through interface 0 for cost 1
    ...
    #add all the links
    link_layer.add_link(link.Link(host1, 0, router_a, 1))
    link_layer.add_link(link.Link(host2, 0, router_a, 0))
    ...
    host1.udt_send(3, 'Sample host1 data %d' % 1)
    sleep(simulation_time)
    host3.udt_send(1, 'Reply host3 data %d' % 1)
